...
class ServiceProcezorExampleWithSalaryBonusBarterTest : Spek({
    fun getTargetsFromDb(period: IPeriod): ITermTargetList {
        ...
        val targets  = arrayListOf<ITermTarget>(
            ...
            ExampleTermTarget(montCode, contract, position, variant1,
                ArticleCode.get(ExampleArticleConst.ARTICLE_PAYMENT_BONUS.code),
                ConceptCode.get(ExampleConceptConst.CONCEPT_AMOUNT_FIXED.code)),
            ExampleTermTarget(montCode, contract, position, variant1,
                ArticleCode.get(ExampleArticleConst.ARTICLE_PAYMENT_BARTER.code),
                ConceptCode.get(ExampleConceptConst.CONCEPT_AMOUNT_FIXED.code)),
        )
        return targets
    }
    ...
    describe("Procezor Example - Salary-Bonus-Barter") {
        ...
        val testArticles = intArrayOf(80001, 80003, 80004, 80002, 80006, 80007, 80010, 80012, 80008, 80009, 80011, 80013);
